<template>
  ...
  export default {
    ...
    async mounted() {
      this.getEmployer(this.$route.params.id);
    },
    methods: {
      getEmployer(id) {
        const user = JSON.parse(localStorage.getItem("user"));
        EmployerService.get(user.id, id)
          .then((response) => {
            console.log(response);
            this.currentEmployer = response.data;
          })
          .catch((e) => {
            console.log(e);
          });
      ...
      updateEmployer() {
        ...
        EmployerService.update(
          user.id,
          this.currentEmployer.id,
          this.currentEmployer
        )
          .then((response) => {
            console.log(response.data);
            this.message = "The employer was updated successfully!";
